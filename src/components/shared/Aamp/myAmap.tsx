...
export default function MapContainer({ ref, makerClick }: any) {
    ...
    useEffect(() => {
        if (mapRef.current && !mapInstance) {
            AMapLoader.load({
              ...
              .then((AMap) => {
                  mapInstance = new AMap.Map("aaContainer", {
                      zoom: 16, // 初始化地图级别
                      features: ['bg', 'point'],
                  ...
                  mapInstance.setLimitBounds(bounds);
  ...
  <div class="water-drop-dot"></div>
    ...
    const jumpToLocation = async (locations: any[], address: string) => {
        ...
        mapInstance.setCenter([locations[0].longitude, locations[0].latitude]);
    ...
    const createNearbyMarks = (posList: any[], center?: string) => {
        center && mapInstance.setCenter(center.split(',').map(Number));
